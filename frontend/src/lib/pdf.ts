...
//   // const fontBase64 = await loadFont('/fonts/segoeuithis.ttf');
//   // doc.addFileToVFS('segoeuithis.ttf', fontBase64);
//   doc.addFont('segoeuithis.ttf', 'SegoeUI', 'normal');
//   doc.setFont('SegoeUI');
...
//   groupedData.value.forEach((city, index) => {
//     if (index > 0) doc.addPage();
//     addHeader(city.name);

//     const tableRows = city.items.map((item) => [
//       item.n_colis + '/' + item.nbr_colis,
//       item.nom_dest,
//       item.ville_dest,
//       item.telephone_dest,
//       item.telephone_exp,
...
//     const tableColumns = ['N°', 'Nom dest.', 'Ville des.', 'Téléphone dest.', 'Téléphone exp.', 'Payée'];
